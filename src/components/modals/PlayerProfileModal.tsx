...
import { X, GraduationCap, Sparkles, ArrowLeft, RefreshCw } from 'lucide-react';
...
export function PlayerProfileModal() {
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 overflow-y-auto bg-black/90 backdrop-blur-md p-3 sm:p-6 sm:py-10 flex justify-center items-start min-h-screen select-none text-white"
    >
        ...
        <div className="flex items-center justify-between pb-4 mb-4 border-b border-[#171717]">
          <div className="flex items-center gap-2">
            ...
            <span className="hidden sm:inline-flex text-xs font-mono font-bold text-[#D4AF37] uppercase items-center gap-1.5 px-3 py-1 rounded-full bg-[#D4AF37]/15 border border-[#D4AF37]/40">
              ...
              PLAYER CARD 2.0 &bull; INTERACTIVE 3D
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5 flex flex-col items-center gap-3">
            <div
              ...
              className="relative w-full max-w-[320px] min-h-[440px] rounded-3xl p-1 bg-gradient-to-b from-[#FFFFFF] via-[#D4AF37] to-[#171717] shadow-[0_0_50px_rgba(212,175,55,0.35)] cursor-pointer"
            >
              {/* Front of Card */}
              {!isFlipped ? (
                <div className="bg-[#0D0D0D] rounded-[22px] p-6 text-white flex flex-col items-center justify-between min-h-[432px] relative overflow-hidden">
                  ...
                  <div className="mt-4 pt-3 border-t border-[#171717] w-full text-center">
                    <span className="text-[10px] font-mono text-[#D4AF37] tracking-widest uppercase">
                      &ldquo;BUILD. SHIP. IMPROVE.&rdquo;
                    </span>
                  </div>
                </div>
              ) : (
                /* Back of Card: Technical Profile */
                <div
                  style={{ transform: 'rotateY(180deg)' }}
                  className="bg-[#0D0D0D] rounded-[22px] p-6 text-white flex flex-col items-center justify-between min-h-[432px] relative overflow-hidden"
                >
                  <div className="w-full border-b border-[#262626] pb-2 text-center">
                    <div className="text-xs font-mono font-bold text-[#D4AF37] tracking-widest uppercase">
                      TECHNICAL PROFILE
                    </div>
                  </div>

                  <div className="w-full space-y-2 text-xs font-mono my-auto">
                    {[
                      { name: 'Python', role: 'AI / Backend Core' },
                      { name: 'JavaScript / TypeScript', role: 'Full Stack Web' },
                      { name: 'React / Next.js', role: 'Interactive UI Architecture' },
                      { name: 'Node.js / Express', role: 'High-Throughput APIs' },
                      { name: 'FastAPI', role: 'High-Performance REST' },
                      { name: 'MongoDB / MySQL', role: 'Scalable Persistence' },
                      { name: 'AWS Cloud', role: 'Cloud Architecture' },
                      { name: 'Docker / CI/CD', role: 'Container Pipelines' },
                    ].map(item => (
                      <div key={item.name} className="flex items-center justify-between p-1.5 rounded bg-[#171717]/80">
                        <span className="text-white font-bold">{item.name}</span>
                        <span className="text-[10px] text-[#D4AF37]">{item.role}</span>
                      </div>
                    ))}
                  </div>

                  <div className="text-[10px] font-mono text-slate-400 border-t border-[#262626] pt-2 w-full text-center">
                    CLICK TO FLIP BACK
                  </div>
                </div>
              )}
            ...
            <button
              onClick={() => setIsFlipped(!isFlipped)}
              className="flex items-center gap-1.5 text-xs font-mono text-[#D4AF37] hover:text-white transition-colors cursor-pointer"
            >
